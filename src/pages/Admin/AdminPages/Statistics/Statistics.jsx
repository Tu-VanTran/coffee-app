...
function Statistics() {
    ...
    const data = orderState.data.filter(item => item.status === status && new Date(startDate) < new Date(item.date) && new Date(item.date) < new Date(endDate))
        ...
        data.map((item, index) => {
            ...
            if (item.status === 'Đã nhận') {
                item.listProductOrder.map((element) => {
                    if (element.type === 'coffee' && element.size.label === 'Nhỏ') {
                        count1 += element.count
                        setCfSmall(count1)
                        count7 += element.total
                        setTotalCf(count7)
                    }
                    if (element.type === 'coffee' && element.size.label === 'Vừa') {
                        count2 += element.count
                        setCfMedium(count2)
                        count7 += element.total
                        setTotalCf(count7)
                    }
                    if (element.type === 'coffee' && element.size.label === 'Lớn') {
                        count3 += element.count
                        setCfLarge(count3)
                        count7 += element.total
                        setTotalCf(count7)
                    }
                    if (element.type === 'other' && element.size.label === 'Nhỏ') {
                        count4 += element.count
                        setOtSmall(count4)
                        count8 += element.total
                        setTotalOt(count8)
                    }
                    if (element.type === 'other' && element.size.label === 'Vừa') {
    ...
    const columns = [
       ...
       
        {
            ...
            key: 'status',
            dataIndex: 'status',
            render: (status) => <div style={status === 'Đã nhận' ? { color: "rgb(3, 145, 3)" } : { color: 'red' }}>{status}</div>
        },
        {
            title: 'TotalBill',
            key: 'totalBill',
        ...
        <div className="statistics-page">
            ...
            <div className="statistics">
                <div className="search">
                    ...
                    {data && status === 'Đã nhận' && <button onClick={showRevenueTotal}>Revenue</button>}
                </div>
