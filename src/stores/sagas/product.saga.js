...
import {fetchProductActionAdmin,fetchProductActionAdminSuccess,
	fetchProductActionAdminFailed, addProductAction, 
	addProductFailed, addProductSuccess, deleteProductAction,
	 deleteProductSuccess, updateProductAction, updateProductFailed,
	updateProductSuccess, searchProductActionAdmin,
	searchProductActionAdminSuccess,
	searchProductActionAdminFailed,
	deleteProductFailed} from "../slices/admin.product.slice";
import { fetchCategoryAction, fetchCategoryActionSuccess, fetchProductAction, fetchProductActionError, fetchProductActionSuccess, searchProductAction, searchProductActionSuccess, searchProductActionFailed } from "../slices/product.slice";
...
function* fetchProductAdmin(action) {
	try {
		...
		yield put(fetchProductActionAdminSuccess({
			data: productData,
			totalProduct: totalProduct
		}))
	} catch (error) {
...
function* searchProductAdmin(action){
	try {
		const response = yield ProductAPI.searchProductList(action.payload)
		const productData = response.data
		yield put(searchProductActionAdminSuccess({
			search: productData
		}))
	} catch (error) {
